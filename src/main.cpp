/********************************************************************
 *  ESP32 Balance-Bot - inner/outer-loop PID + Wi-Fi three-button UI *
 *  With active-state colour feedback                               *
 ********************************************************************/
#include <Arduino.h>
#include <SPI.h>
...
const float REFERENCE_ANGLE = -0.045;  // rad

// --- dynamic Kd boost ---
const float ERROR_SMALL_THRESHOLD = 0.005; // rad
const float GYRO_SPIKE_THRESHOLD  = 0.2;   // rad/s
const float Kd_BOOST_FACTOR       = 100.0;

// ======================  OUTER LOOP  =============================
const float Kp_outer = 0.3;

// ======================  MANUAL DRIVE  ===========================
const float TILT_MANUAL = 0.01f; // rad (~4.6°) extra tilt while button held
...
// Desired position from numeric form (kept for compatibility)
volatile float g_webDesired = 0.0f;

// --- manual-drive & freeze flags ---
...
volatile float freezePositionPos = 0.0f;    // position at stop

volatile float g_positionEstimate = 0.0f;   // shared with /cmd handler

/* =================================================================
 *  HTML PAGE – raw-string literal; the tag ==== ensures the page’s
 *  own ')"' text does NOT terminate the C++ string prematurely.
 * ================================================================= */
...
<script>
/* one-liner fetch helper */
function send(cmd){ fetch('/cmd?act='+cmd); }

/* -----------------------------------------------------------
   bind()
   • sends the start command immediately
   • while the button is held, re-sends it every 200 ms
     (keeps the bot in manual mode even on lossy Wi-Fi)
   • on release, clears the repeat timer and sends "stop"
   ----------------------------------------------------------- */
function bind(id,start){
  ...
  const down=e=>{
    ...
    send(start);                      // 1️⃣ instant command
    repeat=setInterval(()=>send(start),200); // 2️⃣ keep-alive
  };
  ...
  const up=e=>{
    ...
    send('stop');                     // 3️⃣ release → freeze
  };

  /* pointer events cover mouse + touch + pen: nice and tidy */
...
void handleCmd() {
  if (!server.hasArg("act")) { server.send(400,"text/plain","Bad Request"); return; }
...
void setup() {
  ...
  Serial.println("=== ESP32 BalanceBot ===");

  WiFi.softAP("ESP32_BalanceBot","12345678");
  Serial.print("Connect to http://"); Serial.println(WiFi.softAPIP());
  ...
  if (!mpu.begin()) { Serial.println("MPU6050 not detected!"); while (1) delay(10); }
  mpu.setAccelerometerRange(MPU6050_RANGE_2_G);
  mpu.setGyroRange(MPU6050_RANGE_250_DEG);
  mpu.setFilterBandwidth(MPU6050_BAND_44_HZ);

  if (!ITimer.attachInterruptInterval(STEPPER_INTERVAL_US, TimerHandler)) {
    Serial.println("Failed to attach stepper ISR"); while (1) delay(10);
  }
...
void loop() {
  ...
  static float theta=0, integral=0, uout=0;
  static float tilt_acc_z=0, gyro_y=0, err_inner=0;

  /* --- OUTER LOOP ------------------------------------- */
  static unsigned long outerT=0;
  static float posEst=0, desiredPos=0, tiltSP=0;
  ...
  if (now-innerT>=INNER_INTERVAL) {
    ...
    g_positionEstimate = posEst;         // share with /cmd
    ...
    theta = (1-c)*tilt_acc_z + c*(theta + gyro_y*dt);
    ...
    if ((fabsf(err_inner)<ERROR_SMALL_THRESHOLD) &&
        (fabsf(gyro_y)    >GYRO_SPIKE_THRESHOLD)) Kd_eff*=Kd_BOOST_FACTOR;
